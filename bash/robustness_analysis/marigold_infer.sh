...
for perturb_type in ${perturb_list[@]}; do
    ...
    for dataset_name in ${dataset_list[@]}; do
        ...
        for max_iter in ${max_iter_list[@]}; do
            root_save_dir="/path/to/experiment_results/robustness_analysis/${model_name}_modified/${max_iter}_iter/"
            CUDA_VISIBLE_DEVICES=$cuda_device python3 -m src.scripts.marigold_infer \
             --checkpoint_path $checkpoint_path \
             --dataset $dataset_name --min-depth $min_depth --max-depth $max_depth --alignment "least_square"\
