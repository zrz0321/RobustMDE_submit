...
model_name="robustdepth"
encoder_list=("vit" "resnet")
...
for perturb_type in ${perturb_list[@]}; do
    ...
    for dataset_name in ${dataset_list[@]}; do
        ...
        for encoder in ${encoder_list[@]}; do
            ...
            checkpoint_path="/path/to/checkpoints/robust_depth/${encoder}/"
            for max_iter in ${max_iter_list[@]}; do
                root_save_dir="/path/to/experiment_results/robustness_analysis/${model_name}/${encoder}/${max_iter}_iter/"
                CUDA_VISIBLE_DEVICES=$cuda_device python3 -m src.scripts.robustdepth_infer \
                --checkpoint_path $checkpoint_path --encoder $encoder\
