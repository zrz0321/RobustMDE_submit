# ablation study for different optimize method
cuda_device=0
# perturb_list=("color_shift" "geometric" "motion_blur" "banding")
perturb_list=("color_shift")
model_name="marigold"
max_iter=30
# optimize_method_list=("pareto_direct" "direct" "dual_annealing" "original_direct")
optimize_method_list=("pareto_direct")
seed=42
reduced_bit_depth=5
kernel_size=7
...
for perturb_type in ${perturb_list[@]}; do
    ...
    for optimize_method in ${optimize_method_list[@]}; do
        if [ "$optimize_method" == "original_direct" ]; then
            max_batch_num=16
        elif [ "$optimize_method" == "pareto_direct" ]; then
            max_batch_num=16
        else
            max_batch_num=1
        fi

        root_save_dir="/path/to/experiment_results/ablation_study/optimize_method/optimize_method_${optimize_method}/${perturb_type}/"
