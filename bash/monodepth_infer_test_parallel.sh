...
model_type_list=("mono_1024x320" "mono_640x192")
...
for perturb_type in ${perturb_list[@]}; do
    ...
    for dataset_name in ${dataset_list[@]}; do
        ...
        for model_type in ${model_type_list[@]}; do
            checkpoint_path="/path/to/checkpoints/monodepth/${model_type}/"
            ...
            for max_iter in ${max_iter_list[@]}; do
                ...
                root_save_dir="/path/to/experiment_results/DEBUG/${model_name}/${model_type}/${max_iter}_iter/"
                CUDA_VISIBLE_DEVICES=$cuda_device python3 -m src.scripts.monodepth_infer \
