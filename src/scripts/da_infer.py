    ...
    for i, batch in data_iter:
        ...
        with torch.no_grad():
            ...
            current_metric = {
                ...
                "rgb_image_path": batch['image_path'],
            ...
            save_tool.save_as_npy(depth, save_tool.save_dir, "gt_depth")
            ...
            #     plt.ylabel("metric")
            #     plt.title(f"Convergence curve for {perturb_type} on {args.dataset}, using {args.optimize_method}")
            #     save_tool.save_plot(plt, name=f"convergence_curve_{perturb_type}_{args.optimize_method}.png")
            #     plt.close()

            # # draw difference map
            # depth_diff = torch.abs(depth_pred - depth).squeeze().cpu().numpy()
