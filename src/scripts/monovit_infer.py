...
from src.inference_wrapper.monovit.monovit_pipeline import monovitEstimator
...
args.add_argument("--model_type", type=str, default="640x192",
                    help="model type for monovit model. Choices: '640x192', '1024x320'. Default '640x192'.",
                    choices=["640x192", "1024x320"])
...
if __name__ == "__main__":
    ...
    if args.model_type == "1024x320":
        hr = True
        img_size = (320, 1024)
    elif args.model_type == "640x192":
        hr = False
        img_size = (192, 640)
    else:
        raise ValueError("Invalid model_type. Choices: '640x192', '1024x320'.")
    ...
    monovit_model = monovitEstimator(
    ...
    for i, batch in data_iter:
        ...
        with torch.no_grad():
            ...
            depth_pred_original = monovit_model.infer(img)  # depth_pred_original: 1, C, W
            ...
            if max_batch_num > 1 and args.optimize_method in ["original_direct", "pareto_direct"] and inf_pos[0] >= 0 and inf_pos[1] > 1:
                ...
                depth_pred = monovit_model.batch_infer(input_image)[position:position+1, :]
            else:
                depth_pred = monovit_model.batch_infer(input_image)
