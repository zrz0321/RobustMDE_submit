...
args.add_argument("--model_type", type=str, default="mono_1024x320",
                    help="model type for monodepth model. Choices: 'mono_640x192', 'mono_1024x320'. Default 'mono_1024x320'.",
                    choices=["mono_640x192", "mono_1024x320"])
...
if __name__ == "__main__":
    ...
    if args.model_type == "mono_1024x320":
        img_size = (320, 1024)
    elif args.model_type == "mono_640x192":
        img_size = (192, 640)
    else:
        raise ValueError("Invalid model_type. Choices: 'mono_640x192', 'mono_1024x320'.")
    ...
    robustdepth_model = monodepthEstimator(
    ...
    for i, batch in data_iter:
        ...
        with torch.no_grad():
            ...
            depth_pred_original = robustdepth_model.infer(img)  # depth_pred_original: 1, C, W
            ...
            if max_batch_num > 1 and args.optimize_method in ["original_direct", "pareto_direct"] and inf_pos[0] >= 0 and inf_pos[1] > 1:
                ...
                depth_pred = robustdepth_model.batch_infer(input_image)[position:position+1, :]
            else:
                depth_pred = robustdepth_model.batch_infer(input_image)
