...
from src.inference_wrapper.robustdepth.robustdepth_dataset import KITTI, NYUD, HYPERSIM
import torch
from tqdm import tqdm

# encoder = 'vit'
# checkpoint_path="/path/to/checkpoints/robust_depth/vit"
encoder = 'resnet'
checkpoint_path="/path/to/checkpoints/robust_depth/resnet"
min_depth = 0.1
max_depth = 80.0
# _scale_or_not = False
_scale_or_not = True
...
with torch.no_grad():
    ...
    def get_model_para_nums(model):
        ...
        return num_params
    total_params = get_model_para_nums(robustdepth_model.encoder) + get_model_para_nums(robustdepth_model.depth_decoder)
    if total_params >= 1e9:
    ...
    for i, sample in tqdm(enumerate(dataloader)):
        ...
        # Forward pass
        depth_pred = robustdepth_model.batch_infer(image)
...
# python3 -m src.inference_wrapper.robustdepth.robustdepth_test
