...
min_depth = 0.1
max_depth = 80.0
# checkpoint_path="/path/to/checkpoints/monovit/640x192"
# hr=False
checkpoint_path="/path/to/checkpoints/monovit/1024x320"
hr=True
img_size = (192, 640)
...
with torch.no_grad():
    ...
    generator = torch.Generator(device=torch.device("cpu")).manual_seed(42) if 42 is not None else None
    valloader = torch.utils.data.DataLoader(valset, batch_size=1, pin_memory=True, num_workers=4, drop_last=True,
                           shuffle=True, generator=generator)

    # Load model and preprocessing transform
    monodepth_model = monovitEstimator(
    ...
    if not hr:
        ...
        total_params = get_model_para_nums(monodepth_model.depth_decoder)
...
# python3 -m src.inference_wrapper.monovit.monovit_test