...
args.add_argument('--encoder', default='vitb', choices=['vits', 'vitb', 'vitl', 'vitg'])
args.add_argument('--dataset', default='hypersim', choices=['hypersim', 'kitti', 'nyud'])
args.add_argument('--img-size', default=518, type=int)
...
if __name__ == "__main__":
    ...
    if args.dataset == 'hypersim':
        valset = Hypersim('external/Depth_Anything_V2/metric_depth/dataset/splits/hypersim/val.txt', 'val', size=(args.img_size, args.img_size))
        # pass
    elif args.dataset == 'kitti':
        valset = KITTI('external/Depth_Anything_V2/metric_depth/dataset/splits/kitti/val.txt', 'val', size=(args.img_size, args.img_size))
    elif args.dataset == 'nyud':
        valset = NYUD('external/Depth_Anything_V2/metric_depth/dataset/splits/nyud-v2/val.txt', 'val', '/path/to/datasets/nyu_depth_v2', size=(args.img_size, args.img_size))
    else:
    ...
    for i, batch in data_iter:
        ...
        with torch.no_grad():
            ...
            depth_pred_original = da_model.infer(img)  # depth_pred_original: 1, C, W
            ...
            perturb_image = perturb.apply(img, **params_dict)
            input_image = perturb_image.clone()
            ...
            if max_batch_num > 1 and args.optimize_method in ["original_direct", "pareto_direct"] and inf_pos[0] >= 0 and inf_pos[1] > 1:
                ...
                depth_pred = da_model.batch_infer(input_image)[position:position+1, :]
            else:
                depth_pred = da_model.batch_infer(input_image)
            ...
            if args.alignment == "least_square" or args.alignment == "least_square_disparity":
                if perturb_type == "geometric":
                    depth_pred = align_pred_with_gt(depth_pred, gt_depth_perturbed, valid_mask, alignment=args.alignment, align_min_depth=args.min_depth, align_max_depth=args.max_depth)
                else:
                    depth_pred = align_pred_with_gt(depth_pred, depth, valid_mask, alignment=args.alignment, align_min_depth=args.min_depth, align_max_depth=args.max_depth)
                depth_pred_original = align_pred_with_gt(depth_pred_original, depth, valid_mask, alignment=args.alignment, align_min_depth=args.min_depth, align_max_depth=args.max_depth)
            ...
            # draw difference map
            if perturb_type != "geometric":
                depth_diff = torch.abs((depth_pred - depth) * valid_mask).squeeze().cpu().numpy()
                depth_diff[~valid_mask.squeeze().cpu().numpy()] = 0
            else:
                depth_diff = torch.abs((depth_pred - gt_depth_perturbed) * valid_mask).squeeze().cpu().numpy()
                depth_diff[~valid_mask_perturbed.squeeze().cpu().numpy()] = 0
            save_tool.save_heat_map(depth_diff, name=f"depth_difference_{perturb_type}_{args.optimize_method}.png")

            # save perturbed images
            img_path = os.path.join(base_saved_dir, perturb_type, f"{total}", "plots")
            save_image = transform_func(perturb_image.squeeze(0), mode="to").cpu().numpy().transpose(1, 2, 0)
            plt.imsave(os.path.join(img_path, f"perturbed_image.png"), save_image)
    ...
    print(f"Final {target_function} linear: {metric/total}")
