...
for perturb_type in ${perturb_list[@]}; do
    ...
    for dataset_name in ${dataset_list[@]}; do
        ...
        for max_iter in ${max_iter_list[@]}; do
            root_save_dir="/path/to/experiment_results/DEBUG"
            CUDA_VISIBLE_DEVICES=$cuda_device python3 -m src.others.output_evaluation_times \
             --pretrained-from $checkpoint_path \
