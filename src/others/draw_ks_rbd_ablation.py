...
idx = 10
save_root = "./src/others/tmp/ks_rbd_ablation"
...
for typpe in type_list:
    if typpe == "ks":
        perturb_type = "motion_blur"
        additional = "kernel_size"
        additional_list = [3, 5, 7, 9, 11]
    elif typpe == "rbd":
        perturb_type = "banding"
        additional = "reduced_bit_depth"
        additional_list = [5, 6, 7]
    perturb = Perturbation(perturb_type, transform_func=transform_func)
    for value in additional_list:
        record_path = f"/path/to/experiment_results/ablation_study/ks_and_rbd/{additional}_{value}/nyud/{perturb_type}/{idx}/config_pareto_direct.json"
        ...
        for i, data in enumerate(valloader):
            if i < idx:
                ...
                save_dir = os.path.join(save_root, f"{additional}_{value}")
